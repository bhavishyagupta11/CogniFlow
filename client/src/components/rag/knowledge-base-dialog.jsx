...
export function KnowledgeBaseDialog({ open, onOpenChange, onDocumentsChanged }) {
    ...
    useEffect(() => {
        if (!open) return;
        const hasProcessing = documents.some(d => !["completed", "failed", "indexed"].includes(d.processingStatus));
                ...
                if (Array.isArray(data.results)) {
                    data.results.forEach((result) => {
                        if (result.status === 409) {
                            toast.error(result.error?.message || "File conflict");
                        }
                        else if (result.status === 201 || result.ok) {
                            toast.success(`${result.document?.originalFilename || "Document"} uploaded.`);
                        }
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            case "completed":
            case "indexed":
                return <CheckCircle2 className="h-3.5 w-3.5 text-[var(--accent-emerald)]"/>;
            case "failed":
                return <AlertCircle className="h-3.5 w-3.5 text-rose-400"/>;
            default:
                return <Loader2 className="h-3.5 w-3.5 animate-spin text-[var(--accent-amber)]"/>;
        }
    };
        ...
        <DialogContent className="rounded-[4px] border border-[var(--panel-border)] bg-[var(--panel-bg)] text-[var(--text-primary)] max-w-4xl max-h-[85vh] flex flex-col shadow-2xl">
          <DialogHeader className="border-b border-[var(--panel-border)] pb-3">
            ...
            <DialogDescription className="font-mono text-xs text-[var(--text-muted)]">
              Upload documents to expand the retrieval index. Supported formats: PDF, TXT, MD.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className={`mt-4 flex-shrink-0 flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-[4px] transition-colors ${isDragging ? "border-[var(--accent-amber)] bg-[var(--accent-amber)]/10" : "border-[var(--panel-border)] bg-[var(--panel-inner)] hover:border-[var(--accent-amber)]/60"}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            ...
            <input type="file" ref={fileInputRef} className="hidden" multiple accept=".pdf,.txt,.md" onChange={handleFileInput}/>
            ...
            <Table>
              ...
              <TableBody>
                {documents.length === 0 ? (
                  ...
                  documents.map((doc) => (
                    <TableRow key={doc.id || doc.document_id} className="font-mono text-xs border-b border-[var(--panel-border)] hover:bg-[var(--panel-bg)] transition-colors">
                      <TableCell className="font-medium flex items-center gap-2">
                        <FileText className="h-3.5 w-3.5 text-[var(--accent-amber)] flex-shrink-0"/>
                        <span className="truncate text-[var(--text-primary)]">{doc.originalFilename || doc.original_filename || doc.filename}</span>
                      </TableCell>
                      <TableCell className="text-[var(--text-secondary)]">{((doc.size || doc.size_bytes || 0) / 1024 / 1024).toFixed(2)} MB</TableCell>
                      <TableCell className="text-[var(--text-muted)]">{new Date(doc.uploadedAt || doc.created_at || Date.now()).toLocaleDateString()}</TableCell>
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-1.5 text-xs">
                            {getStatusIcon(doc.processingStatus || doc.indexStatus)}
                            <span className={doc.processingStatus === "failed" ? "text-rose-400" : "text-[var(--text-secondary)]"}>
                              {getStatusText(doc.processingStatus || doc.indexStatus)}
                            </span>
                          </div>
                          {doc.processingStatus === "failed" && doc.errorMessage && (
                            <div className="text-[10px] text-rose-400 max-w-[200px] truncate" title={doc.errorMessage}>
                              {doc.errorMessage}
                            </div>
                          )}
                          {!["completed", "indexed", "failed", "queued"].includes(doc.processingStatus) && (
                            <Progress value={undefined} className="h-1 w-full mt-1 bg-[var(--panel-border)]"/>
                          )}
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="h-7 px-2 text-rose-400 hover:text-rose-300 hover:bg-rose-950/40 rounded-[2px]" onClick={() => handleDelete(doc.id || doc.document_id)}>
